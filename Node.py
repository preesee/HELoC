        ...
        if self.is_leaf():
            ...
            if attr_names:
                if 'names' in attr_names:
                    token =token + self.node.names[0]
                elif 'name' in attr_names:
                    token =token + self.node.name
                    is_name = True
                else:
                    token =token + self.node.value
            else:
                token = token + name
        else:
            if name == 'TypeDecl':
...
class SingleNode(ASTNode):
    ...
    def get_token(self, lower=True):
        ...
        if self.is_leaf():
            ...
            if name == 'TypeDecl':
                token =token +str( self.node.declname)
            if self.node.attr_names:
                attr_names = self.node.attr_names
                if 'op' in attr_names:
                    if self.node.op[0] == 'p':
                        token =token + self.node.op[1:]
                    else:
                        token =token + self.node.op
        if token is None:
            token = name
        if lower and is_name:
            token = token.lower()
